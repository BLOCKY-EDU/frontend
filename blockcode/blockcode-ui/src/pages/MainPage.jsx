import React from "react";
import "./MainPage.css";
import logo from "../assets/blocky-logo.png";

export default function MainPage() {
  return (
    <div className="main-container">
      <div className="main-box">
        <div className="main-title-text">
          웹 프론트엔드, 어렵다고 느끼셨나요?<br />
          HTML, CSS 아직 낯설고 복잡하셨나요?
        </div>

        <div className="main-sub-text">
          블록키는 초보자도 쉽게 웹 UI를 만들 수 있는<br />
          블록코딩 기반 웹 프론트 개발 학습 플랫폼입니다.
        </div>

        <div className="main-middle-text">그래서 만들었습니다.</div>

        <div className="main-highlight-text">
          블록처럼 쌓아가며 배우는 웹 프론트엔드
        </div>

        {/* 로고 위치 */}
        <img className="main-image" src={logo} alt="logo" />

        <div className="main-card-container">
          <div className="main-card">한국어로 된 직관적인 블록</div>
          <div className="main-card">구조는 ‘화면’ 꾸미기는 ‘스타일’ 블록으로!</div>
          <div className="main-card">드래그만 하면 바로 스타일 코드가 만들어져요</div>
        </div>
      </div>
    </div>
  );
}